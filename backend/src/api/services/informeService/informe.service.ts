...
export class InformeService {
    ...
    async generarInformePDF(tipoInforme: TipoInforme, filtros: FiltrosInformeDto = {}): Promise<{ buffer: Buffer; archivoInfo: ArchivoInformeDto }> {
        try {
            ...
            const tableData = this.construirDatosTabla(datos);
            this.agregarTablaConFallback(doc, tableData);
            ...
            const pdfBuffer = this.generarBufferPdfSeguro(doc);
    ...
    // Construye filas para la tabla del PDF con validaciones y valores por defecto
    private construirDatosTabla(datos: any[]): string[][] {
        return datos.map((inscripcion) => {
            try {
                return [
    ...
    // Intenta agregar la tabla usando autoTable y aplica un fallback simple si falla
    private agregarTablaConFallback(doc: jsPDF, tableData: string[][]): void {
        try {
            ...
            tableData.forEach((row) => {
                ...
                doc.text(rowText.substring(0, 100), 20, yPosition);
                yPosition += 4;
            });
        }
    }

    // Agrega pie de página de forma segura para múltiples páginas
    private agregarPieDePagina(doc: jsPDF): void {
        try {
    ...
    // Genera el buffer del PDF intentando arraybuffer y con fallback a datauristring
    private generarBufferPdfSeguro(doc: jsPDF): Buffer {
        try {
